    ...
    >
      ...
      {isPopular && (
        <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-gradient-to-r from-violet-600 to-purple-600 text-white px-5 py-1 rounded-full text-xs font-bold tracking-widest uppercase shadow-md shadow-violet-700/40">
          {t("CHOOSE PLAN") /* badge reuse - you can change key if needed */}
        </div>
      )}

      <h2
        className={`text-xl font-bold text-center mb-2 ${isPopular ? "text-violet-300" : "text-white"}`}
      >
        {t(title)}
      </h2>
      ...
      <div className="text-center mb-6">
        <span
          className={`text-4xl font-extrabold ${isPopular ? "text-violet-400" : "text-white"}`}
        >
          {price}
        </span>
