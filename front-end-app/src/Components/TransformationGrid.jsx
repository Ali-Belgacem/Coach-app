    ...
    const fetchTransformations = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5000/api/transformations",
        );
        ...
        console.error(
          "Erreur lors de la récupération des transformations:",
          error,
        );
        setTransformations(TransformationData);
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-20 gap-3 text-violet-300">
        ...
        <span>
          {t("Chargement des transformations...") ||
...
const TransformationCard = ({ item, mobile = false }) => {
  ...
  return (
    <div
      className={`group glass-card rounded-2xl overflow-hidden shadow-lg hover:shadow-violet-700/30 transition-all duration-300 hover:-translate-y-1 ${mobile ? "w-64" : "w-full"}`}
    >
      <div className="flex">
        {/* Before Image */}
        <div className="w-1/2 relative overflow-hidden">
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-black/70 backdrop-blur-sm p-2">
            <p className="text-white font-bold text-center text-xs tracking-widest">
              {t("BEFORE")}
            </p>
          </div>
        </div>
      ...
      <div className="p-3 text-center">
        <p className="text-white font-medium">
          {item.clientId || `${t("Client")} #${item.id}`}
        </p>
        <p className="text-violet-400 text-sm">
          {t(item.duration || "12 weeks")}
        </p>
      </div>
    </div>
  );
